...
function portfolioTemplateCsv(firstName = "Your") {
  return [
    ...
    ",,,,,",
    "Symbol,Quantity,Purchase Price,Holding ($),Holding %,",
    "AAPL,8,175,,,",
    "MSFT,5,390,,,",
    "NVDA,12,105,,,",
    "META,4,485,,,",
...
function parsePortfolioCsv(text) {
  ...
  let headerLineIndex = lines.findIndex((line) => {
    ...
    const hasSizing = headers.some((h) => ["shares", "share", "quantity", "qty", "averagecost", "avgcost", "purchaseprice", "purchasecost", "purchasepricepershare", "buyprice", "costbasis", "costper","holding", "holdings", "holding%", "holding$", "weight", "weight%", "portfolio", "portfolio%", "allocation", "amount", "value", "marketvalue", "percent", "percentage"].includes(h));
  ...
  const avgCostIndex = headers.findIndex((h) => ["averagecost", "avgcost", "purchaseprice", "purchasecost", "purchasepricepershare", "buyprice", "costbasis", "costper", "costpershare", "averageprice", "avgprice"].includes(h));
...
function PortfolioPage({ onBack, onAnalyze }) {
  ...
  const holdingCount = industryGroups.reduce((sum, group) => sum + ((group.holdings || []).length), 0);
      ...
      <section className={`portfolio-input-grid-v3 portfolio-tools-panel ${csvAnalysis ? "portfolio-tools-overlay-open" : "portfolio-tools-first-run"} ${manualOpen ? "manual-open" : "manual-closed"}`}>
        ...
        >
          ...
          <div>
            ...
            <p>The CSV reader accepts <b>Symbol</b>, <b>Quantity</b>, and <b>Purchase Price</b>. Eval also still supports the old Ticker/Shares/Average Cost template, fetches current price, calculates live holding value, then creates the weighted portfolio report.</p>
          </div>
      ...
      {csvAnalysis && !csvLoading && (
        <section className="portfolio-report-v3">
          <div className="portfolio-hero-grid-v3">
            ...
            <article className="portfolio-mini-stat-card-v3 portfolio-count-card-v3">
              <span>Holdings</span>
              <strong>{holdingCount}</strong>
              <small>Scored positions</small>
            </article>

            <article className="portfolio-mini-stat-card-v3 portfolio-count-card-v3">
              <span>Industries</span>
              <strong>{industryCount}</strong>
              <small>Diversification groups</small>
            </article>
          </div>

          <div className="portfolio-dashboard-action-row portfolio-dashboard-action-row-five">
            ...
            <button type="button" className="portfolio-action-tile sync" onClick={() => refreshSavedPortfolio({ silent: false, recordValueHistory: true, recordEvalHistory: true })} disabled={!savedHoldings.length || csvLoading}>
              <b><RefreshCw size={16} className={csvLoading ? "spin" : ""}/> Sync</b>
              <small>Refresh prices + scores</small>
            </button>
