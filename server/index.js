...
function chartUnixSeconds(date) {
...
function getLastWednesdays(count) {
  const dates = [];
  const now = new Date();
  ...
  const latestWednesday = new Date(todayUtc);
  latestWednesday.setUTCDate(todayUtc.getUTCDate() - daysSinceWednesday);

  for (let i = count - 1; i >= 0; i -= 1) {
    const d = new Date(latestWednesday);
    d.setUTCDate(latestWednesday.getUTCDate() - i * 7);
    dates.push(d);
  }
...
function closestTradingCandleOnOrBefore(candles, targetDate) {
  const target =
    Date.UTC(
  ...
  const earliestAllowed = target - 6 * 24 * 60 * 60;
  let best = null;

  for (const candle of candles) {
    if (candle.t <= target && candle.t >= earliestAllowed) {
      if (!best || candle.t > best.t) {
        best = candle;
      }
    }
  }
...
app.get("/api/chart/:symbol", async (req, res) => {
  try {
    ...
    const weeks = rangeWeeks[range] || 4;
    const targetWednesdays = getLastWednesdays(weeks);

    const fromDate = new Date(targetWednesdays[0]);
    fromDate.setUTCDate(fromDate.getUTCDate() - 8);
    ...
    url.searchParams.set("from", String(chartUnixSeconds(fromDate)));
    url.searchParams.set("to", String(chartUnixSeconds(toDate)));
    ...
    if (!response.ok) {
      return res.status(response.status).json({
        error: data?.error || "Finnhub chart request failed.",
      });
    }
    ...
    if (!data || data.s !== "ok" || !Array.isArray(data.t) || !Array.isArray(data.c)) {
      return res.json({
        ...
        range,
        points: [],
        message: "No Finnhub candle data returned.",
      });
    }
    ...
    const points = targetWednesdays
      .map((targetDate) => {
        const candle = closestTradingCandleOnOrBefore(candles, targetDate);

        if (!candle || usedTimestamps.has(candle.t)) return null;

        usedTimestamps.add(candle.t);

        return {
          targetDate: chartDateLabel(targetDate),
          date: candle.date,
